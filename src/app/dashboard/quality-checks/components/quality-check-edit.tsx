...
interface QualityCheckEditProps {
  qualityCheck: QualityCheck;
  onSuccess?: () => void;
}

export function QualityCheckEdit({ qualityCheck, onSuccess }: QualityCheckEditProps) {
  const router = useRouter();
  const [isLoading, setIsLoading] = useState(false);
  const [formData, setFormData] = useState({
    type: qualityCheck.type,
    referenceType: qualityCheck.referenceType,
    referenceId: qualityCheck.referenceId,
    checkedBy: qualityCheck.checkedBy || '',
    color: qualityCheck.color || '',
    taste: qualityCheck.taste || '',
    aroma: qualityCheck.aroma || '',
    texture: qualityCheck.texture || '',
    temperature: qualityCheck.temperature?.toString() || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/quality-checks/${qualityCheck.id}`, {
        method: 'PUT',
        headers: {
      ...
      toast.error(error instanceof Error ? error.message : 'Failed to update quality check');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Failed to delete quality check');
      }
      ...
      toast.error(error instanceof Error ? error.message : 'Failed to delete quality check');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Edit Quality Check</h1>
          <p className="text-muted-foreground">
            Update quality inspection information
          </p>
        </div>
        ...
        <div className="lg:col-span-2">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Update the quality inspection details
              </CardDescription>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <p className="mt-1 text-sm font-mono">{qualityCheck.id}</p>
              </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              <Button
                ...
                onClick={() => router.push(`/dashboard/quality-checks/${qualityCheck.id}`)}
              >
